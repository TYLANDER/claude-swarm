...
app.post("/api/tasks", async (req: Request, res: Response) => {
  try {
    ...
    if (budgetStatus.isPaused) {
      res
        .status(503)
        .json({ error: "Budget limit reached, submissions paused" });
      return;
    }
...
app.get("/api/budget", (_req: Request, res: Response) => {
  ...
  const projectedDailyCostCents =
    budgetStatus.dailyUsedCents + activeAgents * avgCostPerAgent;
...
async function spawnAgent(task: AgentTask) {
  ...
  try {
    ...
    await containerAppsClient.jobs.beginStartAndWait(
